...
function draw() {
    ...
    if (gameState == "play") {
        ...
        for (var i = 0; i < shapes2.length; i++) {
            shapes2[i].rotation += 1;
        }
        if (isdash) {
            just.setCollider("rectangle", 0, 0, 100, 100);
        } else {
            just.setCollider("rectangle", 0, 0, just.width, just.height);
        }
...
function createEnimy() {
    if (frameCount % 5 === 0) {
        shape = createSprite(
            Math.round(random(2500, width - 10)),
            Math.round(random(-1000, height - 10))
        );
